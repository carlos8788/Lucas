let frutas = [
    { nombre: "Manzana", precio: -0.6, cantidad: 10 },
...
// function actualizarCanti(arrayFrutas, indice, cantidadNueva) {    if (arrayFrutas.length - 1 <=indice){
//         if(cantidadNueva > 0){
...
for (let i = 0; i < frutas.length; i++) {
    // Condición: Precio mayor que 0 y el índice no mayor que el índice del último elemento
    if (frutas[i].precio > 0 && i <= frutas.length - 1)
     {
        console.log(`Nombre: ${frutas[i].nombre}, Precio: ${frutas[i].precio}, Cantidad: ${frutas[i].cantidad}`);
    }
}